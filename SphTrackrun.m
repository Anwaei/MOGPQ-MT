...
for m = 1:numMC
    ...
    i_ekf = 1;  % EKF
%     filter = filters.get(i_ekf);
%     name   = filter.getName();
    i_ut = 2;  % UKF
%     filter = filters.get(i_ekf);
%     name   = filter.getName();
    
    updatedPosMean_ekf = reshape(updatedStateMeans(:,i_ekf,:), 3, numTimeSteps);
    RMSEState_ekf = sqrt(1/numTimeSteps*sum(sum((updatedPosMean_ekf-sysStates).^2)));
    updatedPosMean_ut = reshape(updatedStateMeans(:,i_ut,:), 3, numTimeSteps);
    RMSEState_ut = sqrt(1/numTimeSteps*sum(sum((updatedPosMean_ut-sysStates).^2)));
    ...
    fprintf('RMSEState_ekf = %.4f, RMSEState_ukf = %.4f, RMSEState_mo = %.4f, RMSEState_so = %.4f\n', ...
        RMSEState_ekf, RMSEState_ut, RMSEState_mo, RMSEState_so);
    
    NEESState_ut = zeros(1,numTimeSteps);
    NEESState_ekf = zeros(1,numTimeSteps);
    NEESState_mo = zeros(1,numTimeSteps);
    NEESState_so = zeros(1,numTimeSteps);
    for kt = 1:numTimeSteps
        % EKF
        err_ekf = updatedPosMean_ekf(:,kt) - sysStates(:,kt);
        cov_ekf = updatedStateCovs(:, :, i_ekf, kt);
        NEESState_ekf(kt) = err_ekf'/cov_ekf*err_ekf;
        % UT
        err_ut = updatedPosMean_ut(:,kt) - sysStates(:,kt);
        cov_ut = updatedStateCovs(:, :, i_ut, kt);
    ...
    JNEESState_ekf = sqrt(log(mean(NEESState_ekf)/confState.Q)^2);
    ...
    fprintf('JNEESState_ekf = %.4f, JNEESState_ut = %.4f, JNEESState_mo = %.4f, JNEESState_so = %.4f\n', ...
        JNEESState_ekf, JNEESState_ut, JNEESState_mo, JNEESState_so);
